...
Position quotidienne arrondie pour la météo (moteur v2), côté app : faite et testée (jest), **pas encore sur le téléphone** (APK `preview` à reconstruire : `expo-location` ~57.0.20 est natif).
- Option « Envoyer ma position approximative (pour la météo) » dans les réglages, désactivée par défaut ; permission `ACCESS_COARSE_LOCATION` seule, demandée par `PermissionsAndroid` ; `ACCESS_FINE_LOCATION`, `ACCESS_BACKGROUND_LOCATION`, `FOREGROUND_SERVICE_LOCATION` retirées via `android.blockedPermissions` (vérifié par `expo prebuild` : `tools:node="remove"`, le manifeste d'expo-location déclare FINE sinon).
- `app/src/location.ts` (pur) : arrondi à 2 décimales avant tout stockage ou envoi, une position par journée locale (la dernière du jour), journées terminées seulement (jamais aujourd'hui), au plus 30 par envoi, effacées du téléphone après un envoi accepté ; désactiver efface tout. Aucune coordonnée dans les messages, l'état (`SyncState.lastLocation`) ou le diagnostic, et `scrubCoordinates` sur tout texte venu d'ailleurs.
- Envoi dans `runSync`, après la santé (`SyncDeps.location`, facultatif) : `POST /ingest/location` `{ days: [{ date, lat, lon }] }`. **Contrat supposé** (route écrite en parallèle par api-backend) ; validé localement dans `api.ts` faute de schéma dans `@sillage/shared`. Quand le schéma partagé existe, remplacer `checkLocationDays` par son `safeParse`. Un 404 donne « route pas encore disponible », positions gardées, synchro santé intacte.
- Tâche de fond : `getLastKnownPositionAsync` (≤ 12 h), jamais de GPS ; aussi relevée aux réveils « rien à faire ». Sur Android 10+, une app en arrière-plan sans `ACCESS_BACKGROUND_LOCATION` peut n'obtenir aucune position : le relevé à l'ouverture de l'app (s'il manque celle du jour) comble le trou. À vérifier sur le téléphone.
...
### Phase 8 : contexte (météo, position, Kp, lune)
Livré côté API et contrat (agent `extensions`, plusieurs sources liées en une exécution). Cadrage : [docs/sources/weather.md](sources/weather.md). Contrat : docs/API.md (`POST /ingest/location`, champ `context`, tâche `context-sync`).
- [x] Table `daily_context` (migration additive `004_weather_location.sql`, le numéro 003 est réservé au moteur v2).
- [x] `POST /ingest/location` (INGEST_TOKEN) : `{ days: [{ date, lat, lon }] }`, arrondi serveur à 2 décimales, bornes validées, champs inconnus refusés.
- [x] Météo Open-Meteo par jour local du lieu (archive au-delà de 14 jours, prévision en deçà), position du jour sinon maison (`HOME_LAT`/`HOME_LON`).
- [x] Kp maximal par journée locale (GFZ Potsdam, historique complet depuis 1932 et récent : pas besoin de NOAA).
- [x] `moonPhase(date)` pure dans `@sillage/shared` (testée sur des nouvelles et pleines lunes connues).
- [x] `GET /day` et `GET /range` : `context` par journée (objet ou `null`), sans coordonnées.
- [x] Tâche `context-sync` (jobs.ts + `api/vercel.json` `45 2 * * *`) et script `context:backfill` (lots d'un an).
- [ ] **Toi** : `HOME_LAT`, `HOME_LON` (et `CONTEXT_TZ` si ≠ Europe/Paris) sur Vercel puis Redeploy (la migration 004 passe au build) ; `DATABASE_URL=<Neon> HOME_LAT=… HOME_LON=… pnpm --filter api context:backfill` ; vérifier quelques journées contre un relevé météo connu.

**Terminé quand :** `daily_context` a une ligne météo + Kp par journée depuis 2019-07-05 et la tâche l'alimente chaque nuit.

**Notes :**
- **Pour `android-app`** : envoyer `POST /ingest/location` avec `{ "days": [{ "date": "YYYY-MM-DD", "lat": 48.8566, "lon": 2.3522 }] }` (1 à 400 jours, date locale du téléphone, même `INGEST_TOKEN` que `/ingest/health`, réponse `{ "upserted": n }`). Arrondir côté téléphone aussi est bienvenu (rien de plus précis ne quitte le téléphone) ; une position « grossière » (`ACCESS_COARSE_LOCATION`) suffit. Renvoyer une journée remplace sa position ; une position arrivée après le calcul de la météo déclenche son recalcul à la tâche suivante (60 journées au plus par nuit ; au-delà, relancer `context:backfill -- --from …`).
- **Pour le moteur v2 (`generative-art`)** : schémas `DailyMetricsWithContext` / `RangeWithContextResponse` et `DayContext` exportés par `@sillage/shared` ; `context` peut être `null` ou n'avoir que certaines valeurs (météo sans Kp, etc.). Fixture `packages/shared/fixtures/context.json` : une entrée par journée de `days.json` (même ordre, avec `date`), avec des `null` et de vrais zéros ; générée par `node scripts/gen-context-fixtures.mjs`. Paramètres visuels proposés dans la fiche de la source. `sunrise`/`sunset` sont des heures locales du lieu (`HH:MM`), pas des instants.
- **Pour `ops-reliability`** : seuil `weather` 72 h **non ajouté** : l'ajout dans `FRESHNESS_RULES` change `/health` (`last_ingest.weather`) et casse des assertions exactes de `ops.test.ts` (l. 169, 185, 218) et de `api/api/_smoke.ts` (l. 168). À faire dans la zone ops : `{ source: "weather", thresholdHours: 72, label: "météo", hint: "Regarde le cron context-sync dans Vercel (Logs) et HOME_LAT / HOME_LON." }`, et `JOB_SOURCES` n'a pas besoin de `context-sync` (le résultat porte deux sources, déjà dans `ingest_log`).
- **Contrat** : `packages/shared/src/index.ts` réexporte `./context` (import sans extension, sinon art et app refusent l'extension `.ts`) ; le test de `packages/shared` passe donc de `node --experimental-strip-types --test` à `tsx --test` (tsx en devDependency). Si le moteur v2 ajoute `src/selection/` de la même façon, garder ce lanceur.
- **Hors zone, minimal** : une ligne `mountContext(app, deps)` + import dans `api/src/app.ts` (après `mountOps` pour que `/ingest/location` soit journalisé, avant les routes de lecture) ; une ligne dans `jobs.ts`, une dans `api/vercel.json` (vérifier que le plan Vercel accepte un 3ᵉ cron) ; `api/test/db.test.ts` et `api/test/postgres-driver.test.ts` lisent désormais la liste des migrations dans le dossier (au lieu de la liste en dur) pour accepter 003 et 004 ; une ligne de vérification de la migration 004 dans `api/api/_smoke.ts`.
- **Non vérifié** : aucun appel réel à Open-Meteo ni au GFZ (hôtes bloqués depuis l'environnement de l'agent) ; les noms de variables viennent du code source de la doc Open-Meteo, le format GFZ d'un client existant. Les fixtures reprennent ces structures avec des valeurs plausibles. Premier essai réel : `pnpm --filter api context:sync`.
- **Limites connues** : le Kp est agrégé dans un seul fuseau (`CONTEXT_TZ`) même en voyage (écart de quelques heures sur un maximum journalier) ; si `HOME_LAT/HOME_LON` changent, les journées « maison » déjà calculées ne sont pas recalculées automatiquement (relancer le backfill).
...
#### Moteur v2 → techniques : Hachures et Pixels
- **Portées** : `art/src/engine-v2/techniques/hachures.ts`, `pixels.ts` (+ tests `*.test.ts`), une ligne chacune dans le registre. Captures : `art/docs/previews/v2/techniques/{hachures,pixels}-*.jpg` (Hachures 15/07, 10/08 pas non mesurés, 02/09 ; Pixels 28/06 nuit de 5 h 50, 12/09, 15/08 pas non mesurés).
- **Hachures** : plan géométrique pur (`hachuresPlan`, repère 1000) tracé à l'identique en canvas et en SVG. Découpe ← pas (profondeur 3 + 4 × centile) ; espacement de chaque rectangle ← pas de l'heure sous son centre (gauche = minuit) ; angle ← direction du vent ; tremblement du stylo ← vitesse du vent ; rayon du disque ← sommeil ; rectangles à l'encre d'accent ← commits (4 % + 22 % × centile des rectangles, 0 si 0 commit). Encre toujours sombre sur papier clair (palette nocturne : rôles inversés) ; accent = couleur la plus vive et contrastée de la palette. SVG prêt à tracer : 300 mm, calques Inkscape par stylo, chemins seuls (hachures découpées par calcul dans leur rectangle et hors de la réserve du disque), pas de clipPath ni de remplissage (papier = fond CSS) ; marque « données manquantes » en segments ; sommeil absent → contour du disque en pointillés.
- **Pixels** : tampon RGBA pur (`pixelsBuffer`, sans DOM), calculé à ≤ 2000 px (≤ 400 en miniature) puis agrandi sans lissage. Intensité g = 1 − centile du sommeil → seuil du tri des lignes, coulures verticales, glissement des 24 bandes horaires (× activité de l'heure), amplitude des blocs, balayage ; bandes de l'image source ← pas horaires, contraste ← centile des pas ; blocs déplacés ← commits (3 par commit, 90 au plus) ; décalage rouge/bleu ← vitesse et sens du vent. PNG seulement.
- **Mesures** (Chromium headless, 1000 px, fil principal) : Hachures ~30 ms (4000 px : ~0,45 s ; SVG 40–70 Ko, < 10 ms), Pixels 130–220 ms (4000 px : ~0,6 s ; miniature 200 px : 6–23 ms). Même empreinte de pixels d'un rendu et d'un rechargement à l'autre ; `capture:v2` au vert.
- **Hors zone, minimal** : `engine-v2.test.ts` attendait exactement `["maree", "attracteur"]` comme techniques portées ; l'assertion accepte maintenant d'autres techniques portées (`arrayContaining`).
- **À valider par toi** : la table traçante réelle (épaisseur des stylos, 300 mm) ; les cases « Techniques à porter » ci-dessus sont à mettre à jour à la fusion des quatre agents.

#### Moteur v2 → techniques

**Vitrail et Constructif** (famille géométrique) : portés, vectoriels, export SVG fidèle.
- **Fichiers** : `art/src/engine-v2/techniques/vitrail.ts`, `constructif.ts`, `voronoi.ts` (Voronoï exact par demi-plans, arêtes intérieures), `vector.ts` (plan de formes en repère 1000 → canvas `drawPlan` et SVG `planToSvg` depuis le **même** plan ; arcs canvas traduits en arcs SVG), `test-utils.ts` (contexte 2D factice, contrôle XML) ; tests `vitrail.test.ts`, `constructif.test.ts`, `vector.test.ts`.
- **Vitrail** : verres semés heure par heure le long d'une spirale (minuit au centre, minuit suivant au bord, un tour) ; nombre de verres ← pas (36 à 156, + 1 par heure), répartition ← pas horaires ou `hourlyOrFallback` ; un verre vif par commit (60 au plus) ; teinte ← heure × température (repli 16 °C) ; lumière ← `moonPhase` de @sillage/shared (intensité ← fraction éclairée, venue de droite si croissante, de gauche si décroissante) ; assombrissement des bords ← nuages (repli 50 %). Distance minimale de 26/1000 entre verres d'heure (pas de verre plus petit que le plomb). Pas absents : profil horaire type, verres désaturés. 78 à 175 polygones selon la journée.
- **Constructif** : grille 4/5/6, disposition (dispersée, diagonale, grappe, damier, bandes), vocabulaire de 4–5 formes parmi 8, grand module 2 × 2 éventuel ← graine ; modules posés ← pas (22 à 67 % des cases) ; disque maître ← sommeil, **sous** les modules, ≤ 13 % de la surface (pointillés si sommeil absent) ; recouvrements (modules agrandis, encre multipliée) ← nuages ; teinte dominante ← température ; diagonale : axe ← **cap** du vent (0° = nord, l'esquisse lisait les degrés comme un angle mathématique), épaisseur ← vitesse, décalage perpendiculaire ← graine (sinon la même ligne tous les jours sans météo) ; petits carrés ← commits (80 au plus) ; risographie : couche décalée 4 × 3 à 25 %, grain.
- **Mesures** (Chromium headless, 1000 px, fil principal) : Vitrail 45–70 ms, Constructif 30–50 ms ; miniature 200 px (`preview`, sans grain) 1–13 ms ; SVG 3–14 ms, 42–95 Ko (Vitrail), 3–10 Ko (Constructif). Fidélité SVG ↔ canvas hors grain : écart moyen 0,02–0,28 / 255, ≤ 0,05 % de pixels nettement différents (anticrénelage). `capture:v2` : déterministe.
- **Captures** : `art/docs/previews/v2/techniques/` (calme 2026-06-29, normale 2026-07-01, très active 2026-07-22 ; pages du jour avec fiche : 2026-07-04 Vitrail, 2026-07-20 Constructif).
- **Hors fichiers de technique** : deux lignes dans `techniques/index.ts`, liste des techniques portées dans `engine-v2.test.ts` (les autres agents qui portent une technique modifient la même ligne : conflit trivial à la fusion). art/README.md dit encore « Portées : Marée et Attracteur » : à mettre à jour à la fusion.

#### Moteur v2 → techniques : Corail, Harmonographe (+ robustesse de l'Attracteur)
- **Corail** (`techniques/corail.ts`) : pas → nutriments (900 à 2 700), pas horaires → répartition des nutriments de gauche (6 h) à droite (23 h) (repli : profil type × total), commits → 1 à 5 racines, sommeil → épaisseur, vent → inclinaison de la pousse (le vent pousse vers l'aval : vent d'ouest → vers la droite ; repli 250°, 12 km/h), palette v2. Calcul pur dans le repère 1000 (`coralDrawing`), rendu canvas et SVG (un segment par branche, ~190–290 Ko) à partir de la même liste. Troncs isolés corrigés : poche de nutriments réservée au-dessus de chaque racine, tronc arrêté au sommet de la couronne, arbre sans ramification retiré au dessin ; vérifié sur les 120 fixtures × 3 vents (≥ 12 pointes par arbre, tronc nu < 330 / 1000). Nulls : pas → bourgeons creux, commits → racines en pointillés, sommeil → palette brume, + marque commune. ~60–120 ms à 1000 px.
- **Harmonographe** (`techniques/harmonographe.ts`) : pas → rapport des pendules 1–2, commits → rapport 3–4, sommeil → désaccord et amortissement, direction du vent → orientation, force du vent → rotation lente de la table (0,5 rad à 30 km/h, validée par toi), phases ← seed. Amplitude normalisée sur l'étendue réelle calculée : la figure touche toujours la marge de 80 / 1000 sans jamais la dépasser (testé : toutes les fixtures, nulls, vents extrêmes, miniature). SVG = mêmes points (chemins relatifs au dixième, ~570 Ko). ~45 ms à 1000 px, miniature « preview » (pas de temps doublé) ~10 ms.
- **Attracteur** : l'effondrement du 6 juillet venait d'un chaos *transitoire* (forme pleine sur 30 000 points, puis cycle de 7 points). `coverage` suit maintenant 600 000 points par fenêtres de 50 000 et garde la pire (seuil 8 %, ce qui écarte aussi les simples courbes fermées) ; décalages de `a` inchangés (ordre fixe). Filet au rendu (`accumulate`) : une tranche de 20 000 points tombée sur un cycle est jetée et l'orbite relancée d'un point seedé (grands exports). Couleurs calculées sans allocation : accumulation ~30 % plus rapide. 20 jours des fixtures ont un `a` décalé (certains l'étaient déjà) ; seule l'image change, jamais le style figé.
- **Tests** : `techniques/{corail,harmonographe,attracteur}.test.ts` ; outils communs `techniques/testing.ts` (contexte 2D enregistreur, combinaisons de null). `engine-v2.test.ts` : la liste des techniques portées vérifie maintenant « contient Marée et Attracteur » (les autres agents en ajoutent).
- **Captures** : `art/docs/previews/v2/techniques/` (3 journées : calme 29/06, normale 19/06, très active 22/07 ; planche de miniatures ; pages du jour Corail, Harmonographe, Attracteur du 6/07 ; galerie de juillet). `capture:v2` : 10/10 techniques déterministes ; Worker = fil principal au pixel près.
