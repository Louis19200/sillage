...
- **Harmonographe** (`techniques/harmonographe.ts`) : pas → rapport des pendules 1–2, commits → rapport 3–4, sommeil → désaccord et amortissement, direction du vent → orientation, force du vent → rotation lente de la table (nouveau, 0,5 rad à 30 km/h), phases ← seed. Amplitude normalisée sur l'étendue réelle calculée : la figure touche toujours la marge de 80 / 1000 sans jamais la dépasser (testé : toutes les fixtures, nulls, vents extrêmes, miniature). SVG = mêmes points (chemins relatifs au dixième, ~570 Ko). ~45 ms à 1000 px, miniature « preview » (pas de temps doublé) ~10 ms.
