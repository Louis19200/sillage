 ...
 */
import type { DayContext, StyleId } from "@sillage/shared";
...
export type DayV2 = DayInput & {
  ...
  /** Contexte renvoyé par l'API (phase 8) : météo du jour, prioritaire sur les champs à plat. */
...
export function weatherOf(day: DayV2): WeatherInput | null {
  const c = day.context;
  const cloudPct = num(c?.cloud_mean);
  const w: WeatherInput = {
    temp_min: num(c?.temp_min) ?? num(day.temp_min),
    temp_max: num(c?.temp_max) ?? num(day.temp_max),
    precip_mm: num(c?.precip_mm) ?? num(day.precip_mm),
    wind_kmh: num(c?.wind_max_kmh) ?? num(day.wind_max_kmh),
    wind_dir_deg: num(c?.wind_dir_deg) ?? num(day.wind_dir_deg),
    // L'API donne la couverture nuageuse en % ; le moteur l'attend entre 0 et 1.
    cloud: cloudPct !== null ? cloudPct / 100 : num(day.cloud_cover),
  };
