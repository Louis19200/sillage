...
import { ENGINE_VERSION, freezeStyles, getFrozen, todayIn, toSelectionDay } from "./index";
...
describe("freeze-styles", () => {
  ...
  it("donne les styles de la chaîne partagée (même calcul que le navigateur)", async () => {
    ...
    const chain = computeChain({ days: (await db.getRange(FIRST, LAST)).map(toSelectionDay), to: addDays(TODAY, -3) });
