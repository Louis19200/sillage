...
/** Ce que la sélection lit d'une journée (les champs météo passent s'ils existent un jour). */
export function toSelectionDay(d: DailyMetrics): SelectionDay {
  ...
  return {
    ...
    commits: d.commits,
    temp_max: opt("temp_max"),
    precip_mm: opt("precip_mm"),
    wind_max_kmh: opt("wind_max_kmh"),
  };
}
...
export async function freezeStyles(options: FreezeOptions = {}): Promise<FreezeReport> {
  ...
  const days = (await db.getRange(origin, today)).map(toSelectionDay);
...
export function runFreezeStylesJob(): Promise<FreezeReport> {
  return freezeStyles();
}
